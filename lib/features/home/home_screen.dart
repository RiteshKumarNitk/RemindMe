    ...
    return SafeArea(
      ...
      child: ListView(
        ...
        children: [
          ...
          // Due Now banner — prominent call-to-action when a dose is due
          if (appState.nextDose != null)
            _DueNowBanner(
              entry: appState.nextDose!,
              grace: settings.graceDuration,
              now: now,
              onMarkTaken: () => _markTakenWithUndo(context, appState, appState.nextDose!, l10n),
            ),
...
// ---------------------------------------------------------------------------
// Due Now banner — pulsing call-to-action when a dose is due
// ---------------------------------------------------------------------------

class _DueNowBanner extends StatefulWidget {
  const _DueNowBanner({
    required this.entry,
    required this.grace,
    required this.now,
  ...
  @override
  State<_DueNowBanner> createState() => _DueNowBannerState();
}

class _DueNowBannerState extends State<_DueNowBanner>
  ...
  Widget build(BuildContext context) {
    ...
    final status = entry.effectiveStatus(widget.grace, widget.now);
    final isDueNow = status == DoseStatus.pending &&
        !entry.dose.scheduledAt.isAfter(widget.now.add(const Duration(minutes: 10)));

    if (!isDueNow) return const SizedBox.shrink();
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${entry.medicine.name} • ${entry.medicine.doseLabel}',
